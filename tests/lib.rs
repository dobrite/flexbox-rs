...
fn it_sets_child_dim_with_no_child_height() {
    ...
    let layout = layout(width, height, &root);
    assert!(layout.len() == 2);
    assert_eq!(child_width, layout[1].rect.width);
    assert_eq!(height, layout[1].rect.height);
}
...
fn it_sets_child_dim_with_no_child_width() {
    ...
    let layout = layout(width, height, &root);
    assert!(layout.len() == 2);
    assert_eq!(child_height, layout[1].rect.height);
    assert_eq!(0, layout[1].rect.width);
}
